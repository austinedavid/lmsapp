...
export async function POST(req: Request) {
  ...
  // lets now get the one on one section so we can manipulate the resources field and update it
  //   we will then check if is the teacher that created the one-on-one section that actually want to add the material
  const getSection = await prisma.specialTeacherMerged.findUnique({
    where: { id: sectionId },
    select: {
      resources: true,
      teacherId: true,
  ...
  if (getSection.teacherId !== teacherId)
    return new Response(
